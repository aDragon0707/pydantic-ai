...
!!! note "Deferred tools batch at the end of the step"
    Output and function tools execute in parallel (see [`end_strategy`](output.md#parallel-output-tool-calls)), but deferred tool calls are the documented exception: they are collected during the step and either returned as a single `DeferredToolRequests` batch at the end, or resolved inline by the [`HandleDeferredToolCalls`](#resolving-deferred-calls-with-a-handler) capability handler in one shot.
